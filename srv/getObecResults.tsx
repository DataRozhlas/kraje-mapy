...
for (const year of years) {
    ...
    for (const okres of okresyArray) {
        const url = year === "2024" ? `https://www.volby.cz/appdata/kz2024/odata/okresy/vysledky_okres_${okres.nuts}.xml` : `https://www.volby.cz/pls/kz2020/vysledky_okres?nuts=${okres.nuts}`;
        ...
        const result = xmlDoc.VYSLEDKY_OKRES.OBEC.map((obec) => {
            return {
                ...
                parties: parties.map((party) => {
                    return {
                        name: party.name,
                        votes: obec.HLASY_STRANA.find((hlasy) => hlasy.$.KSTRANA === party.kstrana24)?.$.HLASY ?? undefined,
                    }
                })
            }
        });
    ...
    await Bun.write(`srv/data/obecResults${year}.json`, JSON.stringify(results, null, 2));
